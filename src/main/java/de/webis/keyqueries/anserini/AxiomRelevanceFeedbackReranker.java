 ...
 *
 */
public class AxiomRelevanceFeedbackReranker<T> implements Reranker<T>, RelevanceFeedbackAware<T> {
  private static final Logger LOG = LogManager.getLogger(AxiomRelevanceFeedbackReranker.class);
  ...
  public AxiomRelevanceFeedbackReranker(SearchArgs args) {
    this(
      args.index, args.axiom_index, IndexArgs.CONTENTS,
      args.axiom_deterministic, uniqueInt(args.axiom_seed), uniqueInt(args.axiom_r),
      uniqueInt(args.axiom_n), uniqueFloat(args.axiom_beta), uniqueInt(args.axiom_top),
	  args.axiom_docids, args.axiom_outputQuery, args.searchtweets
	);
  }
  
  public AxiomRelevanceFeedbackReranker(String originalIndexPath, String externalIndexPath, String field, boolean deterministic,
                       long seed, int r, int n, float beta, int top, String docidsCachePath,
                       boolean outputQuery, boolean searchTweets) {
    this.field = field;
    ...
    if (this.deterministic && this.N > 1) {
      if (docidsCachePath != null) {
        if (AxiomRelevanceFeedbackReranker.externalDocidsCache == null) {
          AxiomRelevanceFeedbackReranker.externalDocidsCache = buildExternalDocidsCache(docidsCachePath);
          AxiomRelevanceFeedbackReranker.internalDocidsCache = null;
        }
      } else {
        if (AxiomRelevanceFeedbackReranker.internalDocidsCache == null) {
          String indexPath = externalIndexPath == null ? originalIndexPath : externalIndexPath;
          AxiomRelevanceFeedbackReranker.internalDocidsCache = buildInternalDocidsCache(indexPath, this.searchTweets);
          AxiomRelevanceFeedbackReranker.externalDocidsCache = null;
        }
      }
    } else {
      AxiomRelevanceFeedbackReranker.internalDocidsCache = null;
  ...
  public ScoredDocuments rerank(ScoredDocuments docs, RerankerContext<T> context) {
    ...
    try {
      ...
      return searchTopDocs(nq, context, docs);
    } catch (Exception e) {
      e.printStackTrace();
      return docs;
    }
   ...
   */
  private ScoredDocuments searchTopDocs(Query query, RerankerContext<T> context, ScoredDocuments docs) throws IOException {
    ...
    if(query == null || relevantdocs == null) {
   ...
   */
  private List<String> buildExternalDocidsCache(String docidsCachePath) {
    try {
   ...
   * cache.
   */
  private ScoreDoc[] buildInternalDocidsCache(String indexPath, boolean searchTweets) {
    try {
  ...
  private ScoredDocuments processExternalContext(ScoredDocuments docs, RerankerContext<T> context) throws IOException {
    if (this.externalIndexPath != null) {
      ...
      return searchTopDocs(null, externalContext, docs);
    } else {
      return docs;
    }
  }
    ...
    if (docidSet.size() < targetSize) {
      ...
      if (this.deterministic) { // internal docid cannot be relied due to multi-threads indexing,
        ...
        while (docidSet.size() < targetSize) {
          if (AxiomRelevanceFeedbackReranker.externalDocidsCache != null) {
            String docid = AxiomRelevanceFeedbackReranker.externalDocidsCache.get(random.nextInt(AxiomRelevanceFeedbackReranker.externalDocidsCache.size()));
            Query q = new TermQuery(new Term(IndexArgs.ID, docid));
            TopDocs rs = searcher.search(q, 1);
            docidSet.add(rs.scoreDocs[0].doc);
          } else {
            docidSet.add(AxiomRelevanceFeedbackReranker.internalDocidsCache[random.nextInt(AxiomRelevanceFeedbackReranker.internalDocidsCache.length)].doc);
          }
        }
      } else {
  ...
  @Override
  public String tag() {
    return "AxiomaRelevanceFeedbackReranker(R="+R+",N="+N+",K:"+K+",M:"+M+",beta="+beta+")";
  }
